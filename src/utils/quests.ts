import { getDefaultSkillLevels, sortByIndex } from './helpers';
import { convertExperienceToLevels, convertLevelsToExperience, getExperienceForLevel } from './experience'; // prettier-ignore
...
export function useOptimalQuestOrder(): string[] {
  const { quests, order, getQuestById } = useQuestsContext();
  ...
  for (let i = 0; i < allQuests.length; i++) {
    ...
    if (meetsRequirements(availableQuests[0], optimalOrder, levels)) {
      ...
      console.log(
        'Missing',
        Object.fromEntries(missingReqs.map(req => [req.skill, req.level])),
      );
      console.log(
        'Diff',
        Object.fromEntries(
          missingReqs.map(req => [req.skill, req.level - levels[req.skill]]),
        ),
      );
      console.groupEnd();
...
function meetsPotentialRequirements(
  ...
  //     missingReqs.some(
  //       missing => missing.skill === req.skill && req.level >= missing.level,
  //     ),
  //   )
  // ) {
  //   return false;
